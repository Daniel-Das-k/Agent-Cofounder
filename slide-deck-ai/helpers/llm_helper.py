...
def is_valid_llm_provider_model(
    ...
    # Since we only support Gemini, always require API key
    if not api_key:
        return False
...
def get_langchain_llm(
    ...
    :param provider: The LLM provider. Valid values are `gg` for Google Gemini.
    :param model: The name of the LLM.
    :param max_new_tokens: The maximum number of tokens to generate.
    :param api_key: API key or access token to use.
    :param azure_endpoint_url: Azure OpenAI endpoint URL (not used for Gemini).
    :param azure_deployment_name: Azure OpenAI deployment name (not used for Gemini).
    :param azure_api_version: Azure OpenAI API version (not used for Gemini).
